 ...
 */
function validate(oldPath, newPath, encodedOld, refsOnly) {
  ...
  // Validate Claude project dir
  if (!validateClaude(oldPath, encodedOld, refsOnly)) {
    return false;
  }
...
export async function main(args) {
  ...
  try {
    // Validate
    if (!validate(oldPath, newPath, encodedOld, options.refsOnly)) {
      process.exit(1);
    }
    ...
    // Create backups
    backupDir = createBackup(encodedOld, options.dryRun);
    ...
    // Rename Claude dir
    renameClaudeDir(encodedOld, encodedNew, options.dryRun);

    // Update files
    updateProjectFiles(oldPath, newPath, encodedOld, encodedNew, options.dryRun);
    ...
    // Verify
    if (!verifyClaude(oldPath, encodedNew, options.dryRun)) {
