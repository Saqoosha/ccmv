 ...
 */
export function validateClaude(oldPath, encodedOld, refsOnly) {
  ...
  if (existsSync(oldProjectDir)) {
    ...
    logError(`Claude project directory not found: ${oldProjectDir.replace(homedir(), '~')}/`);
    logError('This project may not have any Claude Code history.');
    return false;
  }
}
 ...
 */
export function createBackup(encodedOld, dryRun) {
 ...
 */
export function renameClaudeDir(encodedOld, encodedNew, dryRun) {
  const oldProjectDir = join(PROJECTS_DIR, encodedOld);
 ...
 */
export function updateProjectFiles(oldPath, newPath, encodedOld, encodedNew, dryRun) {
 ...
 */
export function verifyClaude(oldPath, encodedNew, dryRun) {
