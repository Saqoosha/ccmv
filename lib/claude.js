import { existsSync, readdirSync, readFileSync, mkdirSync, cpSync, rmSync, renameSync } from 'node:fs';
import { join, relative } from 'node:path';
import { homedir, tmpdir } from 'node:os';
import { logInfo, logOk, logError, logBackup, logFile, logReplace, logRename, logDryrun, logRollback } from './logger.js';
import { encodePath, formatTimestamp, writeFilePreserveMtime } from './utils.js';
...
export function updateSessions(oldPath, newPath, dryRun) {
  ...
  for (const file of sessionFiles) {
    ...
    try {
      ...
      // Skip files that can't be read (e.g. temp files, permissions)
      continue;
    }
